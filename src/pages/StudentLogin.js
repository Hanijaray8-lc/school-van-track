...
const [showPassword, setShowPassword] = useState(false);
  ...
  const login = async (e) => {
    e.preventDefault();
    try {
      const res = await axios.post("https://school-van-track.onrender.com/api/students/login", form);

      alert(res.data.msg);

      // Send student info to dashboard
      navigate("/student-dashboard", {
        state: {
          studentId: res.data.student._id,
          studentName: res.data.student.studentName,
          vanNumber: res.data.student.vanNumber,
        },
      });
    } catch (err) {
  ...
  <div className="w-full max-w-sm bg-white rounded-2xl shadow-lg p-6">
    ...
    <form onSubmit={login} className="space-y-4">
      <input
        ...
        className="w-full p-3 border border-[#B1AB86] rounded-xl focus:outline-none focus:ring-2 focus:ring-[#819067]"
      />
  ...
  >
    {showPassword ? (
      ...
      <button className="w-full bg-[#819067] text-white py-3 rounded-xl font-semibold hover:bg-[#6d7d54] transition-colors">
        Login
      </button>
    </form>
